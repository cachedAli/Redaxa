...
export default function FeaturesCard({
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="group flex flex-col items-center text-center relative bg-blue-100 backdrop-blur-sm rounded-2xl p-6 border border-gray-200/60 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-2 will-change-transform"
    >
      {/* Hover Overlay */}
      <div
        className={clsx(
          "absolute inset-0 bg-gradient-to-r rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-0",
          hoverOverlay
        )}
      />
